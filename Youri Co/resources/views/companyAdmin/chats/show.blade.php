...
<div class="container">
    <h3 class="text-center">Chat with {{ $staffMember->name }}</h3>
    <div class="messaging">
        <div class="inbox_msg">
            <div class="">
                ...
                <div class="type_msg">
                    <div class="input_msg_write mx-2 mt-2">
                        <form id="" action="{{ route('company-chat.send') }}" method="POST" enctype="multipart/form-data">
                            @csrf
                            <input type="hidden" name="to_user_id" value="{{ $staffMember->id }}">
...
$(document).ready(function() {
    function fetchMessages() {
        $.ajax({
            url: '{{ route("company-chat.show", $staffMember->id) }}',
            method: 'GET',
            dataType: 'json',
                ...
                if (messages.length === 0) {
                    ...
                    messages.forEach(function(message) {
                        ...
                        if (message.message) {
                            ...
                            messageHtml += `<p class="left mt-2 mx-2" style="background: #05728f;border-radius: 3px;font-size: 14px;margin: 0;color: #fff;padding: 5px 10px 5px 12px;width: 40%;">${message.staff_message}</p>`;
                            if (message.file) {
